...
urls = pd.read_csv("C:\myanmar-website-crawlers\sportmm_urls.csv", header=None)
urls = np.array(urls)
urls = urls.tolist()
shuffle(urls)
page_counter = 0
...
for i in range(0,4446):
    ...
    try:
        ...
        # The headtext of the article in sportmm website is in <div> so extract that
        ...
        if content_container == [] or isinstance(content_container, NoneType):
            ...
            #Reusing code from mmtimes for Sportmm
            ...
            # Sportmm writes the article two times using unicode and Zawgyi codes so we have to delete the
            # zawgyi codes font later!

            with open("C:\myanmar-website-crawlers\sportmm_all_data.csv", "a", encoding='utf-8') as WR:
