...
for csv_num in range(0, 9):

    urls = pd.read_csv(
        "C:\myanmar-website-crawlers\\rfa_urls_news_201" + str(csv_num)+".csv", header=None)
    urls = np.array(urls)
    urls = urls.tolist()
    shuffle(urls)
    ...
    wlist = []
    for i in range(0, len(urls)+1): # until the end of the csv list
        ...
        try:
            ...
            # The headtext of the article in rfa website is in <div> so extract that
            ...
            if headtext_container == [] or isinstance(headtext_container, NoneType):
                ...
                for headtext in headtext_container:
                    extracted_content.append(str(headtext.h1.text))
            ...
            if content_container == [] or isinstance(content_container, NoneType):
                ...
                for content in content_container:
                    ...
                    for entry in test:
                        if isinstance(entry, bs4.NavigableString):
                            ...
                            try:
                                extracted_content.append(entry.text)
                            except:
                                continue
                ...
                with open("C:\myanmar-website-crawlers\\rfa_all_data.csv", "a", encoding='utf-8') as WR:
