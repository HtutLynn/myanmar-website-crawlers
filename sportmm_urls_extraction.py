...
for i in range(0,516):
    ...
    try:
        # pre-build a free container for extracted urls and titles
        urls = []
        titles = []


        # Put the url or website into the variable
        my_url = "https://sportsmyanmar.com/page/" + str(i)

        page_counter += 1

        print("Current page count %r " %(page_counter))

        print("Processing {}".format(my_url))
        ...
        # urls are to be extracted from Sport indexing page for each category
        # All articles are inside a similar named div tag
        # Extract all articles urls and their titles

        articles_container = page_soup.findAll("h3", {"class" : "entry-title"})
        ...
        if articles_container == [] or isinstance(articles_container,NoneType):
            ...
            for article in articles_container:
                article_url =str(article.a['href'])
        ...
        with open("C:\myanmar-website-crawlers\sportmm_urls.csv", "a", encoding='utf-8') as WR:
